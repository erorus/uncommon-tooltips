...
function GetExeVersion($filename) {
    $data = GetExeInfo($filename);

    return $data['FileVersion'] ?? $data['ProductVersion'] ?? false;
}

// https://stackoverflow.com/questions/41228319/extract-internal-exe-info
function GetExeInfo($filename, $encoding = 'UTF-8') {
